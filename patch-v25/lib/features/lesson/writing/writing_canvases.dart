...
class _StrokesPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // في الألف نملأ المساحة الداخلية للدليل المتقطع كاملة، لا نرسم
    // خيطًا رفيعًا في منتصف الحرف.
    final strokeWidth = isAlif
        ? (size.shortestSide * .06).clamp(12.0, 28.0)
        : (size.shortestSide * .035).clamp(8.0, 18.0);
...
/// مرجع الألف المتقطع المعتمد من المستخدم، مع قص المساحة البيضاء حوله
/// وإبقائه في مركز اللوحة على جميع مقاسات iPhone الأفقية.
class _LetterGuideBackdrop extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return IgnorePointer(
      child: LayoutBuilder(
        builder: (context, constraints) {
          final height = constraints.maxHeight * .94;
          return Center(
            child: SizedBox(
              width: height * .30,
              height: height,
              child: ClipRect(
                child: Image.asset(
                  'assets/images/writing/alif_dashed_guide.png',
                  fit: BoxFit.cover,
