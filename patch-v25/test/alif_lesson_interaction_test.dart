...
void main() {
  test('مسار الألف كثيف ويبدأ بالفتحة ثم الهمزة ثم جسم الألف', () {
    ...
    final hamzaUpper = strokes[1].samples(const Size(600, 300), count: 96);
    final hamzaLower = strokes[2].samples(const Size(600, 300), count: 96);
    final body = strokes.last.samples(const Size(600, 300), count: 96);

    expect(fatha.first.dy, lessThan(hamzaUpper.first.dy));
    expect(fatha.last.dx, greaterThan(fatha.first.dx));
  ...
  test('التحقق الصوتي يقبل آ ولا يقبل اسم ألف', () {
    expect(speechMatchesExpected('أَ', 'آ'), isTrue);
    expect(speechMatchesExpected('آ', 'آ'), isTrue);
    expect(speechMatchesExpected('ألف', 'آ'), isFalse);
