  ...
  SceneChannel _channelFor(Scene scene) {
    if (_channelSceneId != scene.id) {
      ...
      final entryPose = switch (scene.type) {
        ...
        // مشهد النهاية يبدأ هادئًا؛ حدثه المتزامن يشغل التشجيع مرة ثم
        // ينتقل إلى التلويح المتكرر.
        SceneType.success => SalehPose.idle,
        _ => SalehPose.idle,
      };
...
class _LessonBoard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final outerPadding = compact ? 7.0 : 13.0;
    final contentTop = outerPadding + (compact ? 30.0 : 42.0);
...
double _lessonActionClearance(BuildContext context) =>
    MediaQuery.sizeOf(context).height < 760 ? 24 : 28;
