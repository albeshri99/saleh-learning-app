...
/// فحص طفولي متسامح للكتابة الحرة لحرف «أَ».
///
/// الغرض من اللوحة الحرة هو تشجيع الطفل، لا اختبار دقة التتبع. لذلك نقبل
/// اختلاف اتجاه الفتحة والهمزة، وترتيب الضربات، وميل جسم الألف، ما دام هناك:
/// جسم ألف طويل وواضح، وحبر قصير أعلى الجسم يدل على الهمزة/الفتحة. نبقي فقط
/// حواجز بسيطة تمنع قبول اللوحة الفارغة أو خط أفقي أو خربشة واضحة.
WritingValidationResult validateAlifFathaChildFriendly(WritingSample sample) {
  ...
  final normalized = sample.strokes
      ...
      .where((stroke) => stroke.length >= 2 && _pathLength(stroke) >= .012)
  ...
  final bodyLike = body.bounds.height >= .27 &&
      body.verticality >= .48 &&
      body.straightness >= .32 &&
      body.absoluteTurn <= 9.5 &&
      body.length / math.max(.001, body.bounds.height) <= 4.2;
  if (!bodyLike) {
    return const WritingValidationResult(
      isValid: false,
      score: .25,
  ...
  final upperInk = <_StrokeFeatures>[];
  for (var i = 0; i < features.length; i++) {
    if (i == bodyIndex) continue;
    final mark = features[i];
    final closeToTop = mark.bounds.center.dy <= body.bounds.top + .23;
    final substantial = mark.length >= .012 &&
        (mark.bounds.width >= .012 || mark.bounds.height >= .012);
    final notASecondScribble = mark.length <= 1.25 && mark.absoluteTurn <= 16;
    if (closeToTop && substantial && notASecondScribble) upperInk.add(mark);
  }
  if (upperInk.isEmpty) {
    return const WritingValidationResult(
      isValid: false,
      score: .46,
      missingParts: ['topMarks'],
      reason: 'missingParts',
    );
  }

  final overall = _unionBounds(features.map((feature) => feature.bounds));
  final totalLength =
      features.fold<double>(0, (sum, item) => sum + item.length);
  final plausibleSize = overall.height >= .30 && overall.height <= .98;
  final notScribble = normalized.length <= 7 && totalLength <= 5.2;
  if (!plausibleSize || !notScribble) {
    return const WritingValidationResult(
      isValid: false,
      score: .34,
      reason: 'sizeOrCenter',
  ...
  final bodyScore = _ramp(body.bounds.height, low: .25, high: .55) * .45 +
      _ramp(body.verticality, low: .42, high: .78) * .35 +
      _ramp(body.straightness, low: .25, high: .72) * .20;
  final score = (.66 + bodyScore * .28 + math.min(upperInk.length, 2) * .03)
      .clamp(0.0, 1.0);
  return WritingValidationResult(isValid: true, score: score);
}
