...
class LetterTraceTemplate {
  ...
  /// ترتيب الدليل المعتمد في النسخة 31: جسم الألف، ثم الهمزة، ثم الفتحة.
  /// تبقى [parts] كما هي حتى لا يتغير رسم القالب أو تصميمه في النسخة 32.
  List<LetterTracePart> get guideParts {
    const order = <String, int>{'body': 0, 'hamza': 1, 'fatha': 2};
    return [...parts]..sort(
        (a, b) => (order[a.id] ?? order.length)
            .compareTo(order[b.id] ?? order.length),
      );
  }
  ...
  List<StrokeSpec> get strokes => guideParts
      .map(
  ...
  List<Offset> samples(Size size, int partIndex, {int count = 40}) {
    final orderedParts = guideParts;
    if (partIndex < 0 || partIndex >= orderedParts.length) return const [];
    return orderedParts[partIndex].samples(drawingRect(size), count: count);
  }
...
///
/// ترتيب التعلّم المعتمد: الفتحة أولًا من أعلاها إلى أسفلها، ثم الهمزة من
/// أعلاها إلى أسفلها، ثم جسم الألف من الأعلى إلى الأسفل. حدود الحرف ثابتة؛
/// ولا تتحرك سوى تعبئة اللون داخلها.
