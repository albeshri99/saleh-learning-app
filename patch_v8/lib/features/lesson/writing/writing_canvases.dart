...
class _StrokesPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final guide = Paint()
      ...
      ..strokeWidth = (size.shortestSide * 0.022).clamp(3.0, 6.0)
      ..color = AppColors.letterGuide;
    final guideBody = Paint()
      ..style = PaintingStyle.stroke
      ..strokeCap = StrokeCap.round
      ..strokeJoin = StrokeJoin.round
      ..strokeWidth = (size.shortestSide * 0.13).clamp(18.0, 34.0)
      ..color = AppColors.letterGuide.withValues(alpha: .24);
    final done = Paint()
      ..style = PaintingStyle.stroke
      ..strokeCap = StrokeCap.round
      ..strokeJoin = StrokeJoin.round
    ...
    // الدليل المنقّط الكامل: يرى الطفل شكل الحرف واتجاهه قبل اللمس.
    for (final s in strokes) {
      ...
      if (s.kind == StrokeKind.dot) {
        ...
        canvas.drawPath(path, guideBody);
    ...
    for (var i = 0; i < strokes.length; i++) {
      ...
      if (progress != null) {
        final per = 1 / strokes.length;
        strokeProgress = ((progress! - i * per) / per).clamp(0.0, 1.0);
      } else if (i < completedStrokes) {
        strokeProgress = 1;
      } else if (i == completedStrokes) {
        strokeProgress = activeStrokeProgress;
      } else {
        strokeProgress = 0;
      }
      ...
      if (s.kind == StrokeKind.dot) {
        if (strokeProgress >= 1) {
          canvas.drawPath(path, Paint()..color = completedColor);
        }
      } else {
        for (final m in path.computeMetrics()) {
          canvas.drawPath(m.extractPath(0, m.length * strokeProgress), done);
        }
      }
    }
