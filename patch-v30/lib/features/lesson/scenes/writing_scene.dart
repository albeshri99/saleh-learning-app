...
class _WritingSceneState extends State<WritingScene> {
  ...
  void _validateFreeWriting() {
    ...
    final result = validateAlifFatha(canvas.sample);
    if (result.isValid) {
      _finishAttempt();
      return;
    }
    ...
    setState(() {
      _freeValidationMessage = switch (result.reason) {
        'missingParts' => 'اكتب جسم الألف والهمزة والفتحة كاملة',
        'strokeOrder' => 'ابدأ بجسم الألف، ثم الهمزة، ثم الفتحة',
        'bodyDirection' => 'اكتب جسم الألف من الأعلى إلى الأسفل',
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        SizedBox(
          height: MediaQuery.sizeOf(context).height < 760 ? 36 : 52,
          child: Row(
          ...
          children: [
            if (!_guided) ...[
              ...
              LessonActionButton(
                ...
                icon:
                    _hasFreeInk ? Icons.check_rounded : Icons.skip_next_rounded,
                onPressed: _hasFreeInk
