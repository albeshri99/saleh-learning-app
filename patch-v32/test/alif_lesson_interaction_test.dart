...
void main() {
  test('قالب الفيديو يبدأ بالفتحة ثم الهمزة ثم جسم الألف', () {
    ...
    final fatha = strokes.first.samples(const Size(600, 300), count: 96);
  ...
  test('المحتوى يربط الترحيب والتتبع والنهاية بالحركات والأصوات المعتمدة', () {
    final lesson = _loadAlif();
    final welcome =
        lesson.scenes.singleWhere((scene) => scene.id == 'welcome_1');
    final guided =
        lesson.scenes.singleWhere((scene) => scene.id == 'write_guided_1');
    final success =
        lesson.scenes.singleWhere((scene) => scene.id == 'success_1');

    expect(welcome.lines.single.events.single.params['name'], 'greetThenTalk');
    expect(
      guided.data['guidedPraiseAudio'],
      'assets/audio/alif/guided_praise.mp3',
    );
    expect(
      success.lines.single.events.single.params['name'],
      'celebrateThenWave',
    );
  });
  ...
  testWidgets('التقويم شفهي ويجعل صالح يفكر ولا ينتظر صوت النجاح',
      (tester) async {
    final lesson = _loadAlif();
    final assessment =
        lesson.scenes.singleWhere((scene) => scene.id == 'assessment_1');
    ...
    await tester.pump();
    expect(salehActions.last, 'thinking');
    expect(find.text('ما الكلمة التي تبدأ بحرف أَ؟'), findsNothing);
