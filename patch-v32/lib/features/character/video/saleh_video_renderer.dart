...
class _SalehVideoRendererState extends State<SalehVideoRenderer> {
  ...
  // WebM ذي الشفافية كان يظهر مستطيلاً أسود/رمادي في بعض المتصفحات.
  // WebP المتحرك هو الأصل الشفاف نفسه المستخدم على iPhone ويمنع الإطار.
  bool get _usesAnimatedAsset =>
  ...
  Widget build(BuildContext context) {
    if (widget.pose == SalehPose.thinking) {
      final width = widget.width ?? 260;
      final height = widget.height ?? 430;
      return SizedBox(
        width: widget.width,
        ...
        child: Stack(
          ...
          children: [
            Positioned.fill(
              child: Saleh2DIdle(width: widget.width, height: widget.height),
            ),
            Positioned(
              top: height * .11,
              right: width * .01,
    ...
    if (_usesAnimatedAsset) {
      ...
      final cacheWidth =
          (((widget.width ?? 260) * dpr).round()).clamp(360, 720);
      final animatedImage = Image.asset(
        ...
        // لا تختفي الشخصية أثناء فك أول إطار من الحركة الكبيرة؛ يظهر
        // الملصق الشفاف فورًا ثم يُستبدل بالحركة عند جاهزيتها.
      ...
      return SizedBox(
        ...
        child: AnimatedSwitcher(
          ...
          // أصول WebP تحمل قناة Alpha جاهزة. تعديل قناة Alpha بمصفوفة لون
          // كان يعيد إظهار بكسلات الخلفية الشفافة كمستطيل رمادي/أسود.
          child: KeyedSubtree(
            key: ValueKey('alpha_${widget.pose.name}'),
            child: animatedImage,
          ),
        ),
      );
