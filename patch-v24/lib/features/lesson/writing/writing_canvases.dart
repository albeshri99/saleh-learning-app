...
class _StrokesPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    TextPainter? alifHamza;
    Offset? alifHamzaOffset;
    if (isAlif) {
      alifHamza = TextPainter(
        text: TextSpan(
          text: 'ء',
          style: TextStyle(
            fontFamily: 'Tajawal',
            ...
            height: 1,
            color: AppColors.letterGuide.withValues(alpha: .70),
          ),
        ),
        textDirection: TextDirection.rtl,
        textAlign: TextAlign.center,
      )..layout();
      alifHamzaOffset = Offset(
        (size.width - alifHamza.width) / 2,
        size.height * .20,
      );
      alifHamza.paint(canvas, alifHamzaOffset);
    }
    ...
    if (isAlif && alifHamza != null && alifHamzaOffset != null) {
      final double hamzaProgress;
      if (progress != null) {
        hamzaProgress = (progress! * strokes.length / 2).clamp(0.0, 1.0);
      } else if (completedStrokes >= 2) {
        hamzaProgress = 1;
      } else if (completedStrokes == 1) {
        hamzaProgress = .5 + activeStrokeProgress * .5;
      } else {
        hamzaProgress = activeStrokeProgress * .5;
      }
      if (hamzaProgress > 0) {
        final tracedHamza = TextPainter(
          text: TextSpan(
            text: 'ء',
            style: TextStyle(
              fontFamily: 'Tajawal',
        ...
        canvas.clipRect(
          Rect.fromLTWH(
            alifHamzaOffset.dx + tracedHamza.width * (1 - hamzaProgress),
            alifHamzaOffset.dy,
            tracedHamza.width * hamzaProgress,
            tracedHamza.height,
          ),
        );
        tracedHamza.paint(canvas, alifHamzaOffset);
        canvas.restore();
      }
    }
